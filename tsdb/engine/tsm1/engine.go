...
type Engine struct {
	...
	id           uint64
	path         string // 对应 shard.path tsm1.engine.go:199
	...
	// specifies the minimum size threshold for
	...
	// maps a series key to field type 环境变量INFLUXDB_SERIES_TYPE_CHECK_ENABLED
...
func NewEngine(id uint64, index tsdb.Index, path string, walPath string, sfile *tsdb.SeriesFile, engineOptions tsdb.EngineOptions) tsdb.Engine {
...
// enable compactions on the engine.  When disabled
...
// will force the engine to fully compact all data stored.
...
// closes the engine. Subsequent calls to Close are a nop.
...
func (engine *Engine) WritePoints(ctx context.Context, points []models.Point) error {
	measurementTagsFieldKey2FieldValues := make(map[string][]Value, len(points)) // measurement+tags+各field的key
	var (
		keyBuf    []byte
		baseLen   int
		seriesErr error
	...
	for _, point := range points {
		keyBuf = append(keyBuf[:0], point.Key()...)
		keyBuf = append(keyBuf, keyFieldSeparator...)
		baseLen = len(keyBuf)
		fieldIterator := point.FieldIterator()
		unixNano := point.Time().UnixNano()
		for fieldIterator.Next() {
			...
			keyBuf = append(keyBuf[:baseLen], fieldIterator.FieldKey()...)
			...
			if engine.seriesTypeMap != nil {
				...
				if v, ok := engine.seriesTypeMap.Get(keyBuf); !ok {
					...
					} else if typ != fieldIterator.Type() {
					...
					vv, ok := engine.seriesTypeMap.Insert(keyBuf, int(fieldIterator.Type()))
					...
					if !ok || vv != int(fieldIterator.Type()) {
						seriesErr = tsdb.ErrFieldTypeConflict
						continue
					}
				} else if v != int(fieldIterator.Type()) {
			...
			switch fieldIterator.Type() {
			case models.Float:
				fv, err := fieldIterator.FloatValue()
				if err != nil {
					return err
				}
				fieldValue = NewFloatValue(unixNano, fv)
			case models.Integer:
				iv, err := fieldIterator.IntegerValue() // []byte->string->数
				if err != nil {
					return err
				}
				fieldValue = NewIntegerValue(unixNano, iv)
			case models.Unsigned:
				iv, err := fieldIterator.UnsignedValue()
				if err != nil {
					return err
				}
				fieldValue = NewUnsignedValue(unixNano, iv)
			case models.String:
				fieldValue = NewStringValue(unixNano, fieldIterator.StringValue())
			case models.Boolean:
				bv, err := fieldIterator.BooleanValue()
				if err != nil {
					return err
				}
				fieldValue = NewBooleanValue(unixNano, bv)
			default:
				return fmt.Errorf("unknown field type for %s: %s", string(fieldIterator.FieldKey()), point.String())
			}
			measurementTagsFieldKey2FieldValues[string(keyBuf)] = append(measurementTagsFieldKey2FieldValues[string(keyBuf)], fieldValue)
		}
	}
	...
	// 先写到cache
	if err := engine.Cache.WriteMulti(measurementTagsFieldKey2FieldValues); err != nil {
		return err
	}

	if engine.WALEnabled {
		if _, err := engine.WAL.WriteMulti(ctx, measurementTagsFieldKey2FieldValues); err != nil {
			return err
		}
	}
	return seriesErr
...
// 严格意义上应该是WriteCacheSnapshot, snapshot the cache ,write it into a new TSM file, releasing the snapshot when done.
func (engine *Engine) WriteSnapshot() (err error) {
	...
	closedSegmentFilePaths, cacheSnapshot, err := func() (closedSegmentFilePaths []string, cacheSnapshot *Cache, err error) {
		engine.mu.Lock()
		defer engine.mu.Unlock()
		...
		if engine.WALEnabled {
			...
			closedSegmentFilePaths, err = engine.WAL.ClosedSegmentFilePaths()
			if err != nil {
				return
			}
		}

		cacheSnapshot, err = engine.Cache.Snapshot()
		if err != nil {
			return
		}
...
func (engine *Engine) writeSnapshotAndCommit(log *zap.Logger, closedSegmentFilePaths []string, cacheSnapshot *Cache) (err error) {
	defer func() {
		if err != nil {
	...
	if engine.WALEnabled {
		if err = engine.WAL.Remove(closedSegmentFilePaths); err != nil {
...
func (engine *Engine) ShouldCompactCache(t time.Time) bool {
	...
	// 空间上确认是不是需要
	if cacheSize > engine.CacheFlushMemorySizeThreshold {
		return true
	}
	// 时间上确认是不是需要
...
func (engine *Engine) reloadCache() error {
	now := time.Now()
	files, err := segmentFilePaths(engine.WAL.Path())
	if err != nil {
		return err
	}
