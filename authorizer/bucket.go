...
func (bucketService *BucketService) FindBucketByID(ctx context.Context, id platform.ID) (*influxdb.Bucket, error) {
	span, ctx := tracing.StartSpanFromContext(ctx)
	defer span.Finish()

	b, err := bucketService.s.FindBucketByID(ctx, id)
	if err != nil {
		return nil, err
	}
...
func (bucketService *BucketService) FindBucketByName(ctx context.Context, orgID platform.ID, n string) (*influxdb.Bucket, error) {
	span, ctx := tracing.StartSpanFromContext(ctx)
	defer span.Finish()

	b, err := bucketService.s.FindBucketByName(ctx, orgID, n)
	if err != nil {
		return nil, err
	}
	if _, _, err := AuthorizeReadBucket(ctx, b.Type, b.ID, b.OrgID); err != nil {
...
func (bucketService *BucketService) FindBucket(ctx context.Context, filter influxdb.BucketFilter) (*influxdb.Bucket, error) {
	span, ctx := tracing.StartSpanFromContext(ctx)
	defer span.Finish()

	b, err := bucketService.s.FindBucket(ctx, filter)
	if err != nil {
		return nil, err
	}
	if _, _, err := AuthorizeReadBucket(ctx, b.Type, b.ID, b.OrgID); err != nil {
...
func (bucketService *BucketService) FindBuckets(ctx context.Context, filter influxdb.BucketFilter, opt ...influxdb.FindOptions) ([]*influxdb.Bucket, int, error) {
	span, ctx := tracing.StartSpanFromContext(ctx)
	defer span.Finish()

	// TODO: we'll likely want to push this operation into the database eventually since fetching the whole list of data
	// will likely be expensive.
	bs, _, err := bucketService.s.FindBuckets(ctx, filter, opt...)
	if err != nil {
		return nil, 0, err
	}
...
func (bucketService *BucketService) CreateBucket(ctx context.Context, b *influxdb.Bucket) error {
	span, ctx := tracing.StartSpanFromContext(ctx)
	defer span.Finish()
	...
	return bucketService.s.CreateBucket(ctx, b)
}

// UpdateBucket checks to see if the authorizer on context has write access to the bucket provided.
func (bucketService *BucketService) UpdateBucket(ctx context.Context, id platform.ID, upd influxdb.BucketUpdate) (*influxdb.Bucket, error) {
	b, err := bucketService.s.FindBucketByID(ctx, id)
	if err != nil {
		return nil, err
	}
	...
	return bucketService.s.UpdateBucket(ctx, id, upd)
}

// DeleteBucket checks to see if the authorizer on context has write access to the bucket provided.
func (bucketService *BucketService) DeleteBucket(ctx context.Context, id platform.ID) error {
	b, err := bucketService.s.FindBucketByID(ctx, id)
	if err != nil {
		return err
	}
	...
	return bucketService.s.DeleteBucket(ctx, id)
}
